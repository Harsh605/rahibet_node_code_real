...
module.exports = function (animal_roulette_socket, request) {
    ...
    animal_roulette_socket.on('connection', (socket) => {
        if (!game_created) {
            ...
            var options = {
                'method': 'GET',
                'url': 'https://admin.luckystar365.in/letscard/api/cron/animal_roulette_create_socket'
            };
    ...
    function timer_function() {
        ...
        if (timer <= 0) {
            ...
            var options = {
                'method': 'GET',
                'url': 'https://admin.luckystar365.in/letscard/api/cron/animal_roulette_winner_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);
                game_created = false;
                ...
                var options = {
                    'method': 'POST',
                    'url': 'https://admin.luckystar365.in/letscard/api/animalRoulette/get_active_game_socket',
                    'headers': {
