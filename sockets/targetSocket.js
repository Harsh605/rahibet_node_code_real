...
module.exports = function (target_socket, request) {
    ...
    target_socket.on('connection', (socket) => {
        if (!game_created) {
            ...
            var options = {
                'method': 'GET',
                'url': 'https://admintesting.rahibet.com/letscard/api/cron/target_create_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);

                // console.log("target_create_socket", response.body);
            });
            interval_id = setInterval(timer_function, 1000);
        }

        socket.on('target_timer', msg => {
    ...
    function timer_function() {
        ...
        if (timer <= 0) {
            ...
            var options = {
                'method': 'GET',
                'url': 'https://admintesting.rahibet.com/letscard/api/cron/target_winner_socket'
            };
            request(options, function (error, response) {
                if (error) throw new Error(error);
                game_created = false;
                ...
                var options = {
                    'method': 'POST',
                    'url': 'https://admintesting.rahibet.com/letscard/api/target/get_active_game_socket',
                    'headers': {
